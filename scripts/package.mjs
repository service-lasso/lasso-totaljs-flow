...
function versionedAssetName(version, platform, archiveType) {
  return `lasso-totaljs-flow-${version}-${platform}.${archiveType === "zip" ? "zip" : "tar.gz"}`;
}
...
export async function packageFlow(platform = targetPlatform, version = serviceVersion) {
  ...
  await writeFile(path.join(packageRoot, "lasso-totaljs-flow.mjs"), launcherSource, "utf8");
  await writeFile(
    ...
    `${JSON.stringify(
      {
        ...
        platform,
        arch: "x64",
        command: "node ./lasso-totaljs-flow.mjs",
      },
      null,
      2,
    )}\n`,
  ...
  if (platform !== "win32") {
    await chmod(path.join(packageRoot, "lasso-totaljs-flow.mjs"), 0o755);
  }

  await compressPackage(packageRoot, outputPath, target.archiveType);
  console.log(`[lasso-totaljs-flow] packaged ${outputPath}`);
  return outputPath;
}
...
const appRoot = path.join(packageRoot, "app");
const servicePort = process.env.SERVICE_PORT || process.env.FLOW_PORT || "8111";
const messagePort = process.env.MESSAGESERVICE_PORT || "8112";

const env = {
  ...process.env,
  NODE_ENV: process.env.NODE_ENV || "production",
  SERVICE_PORT: servicePort,
  FLOW_PORT: servicePort,
  FLOW_HOST_URL: process.env.FLOW_HOST_URL || ` + "`http://127.0.0.1:${servicePort}`" + `,
  MESSAGESERVICE_PORT: messagePort,
  MESSAGESERVICE_URL: process.env.MESSAGESERVICE_URL || ` + "`http://127.0.0.1:${messagePort}`" + `,
};
